...
Internally, `AnalogButton2` owns a fixed-size array of up to `ABS_MAX_BUTTONS` (10) real `Button2` instances. Each instance is created with `BTN_VIRTUAL_PIN` and a lambda state function that returns the button's current `states[i]` byte (`LOW` = pressed, `HIGH` = released). `AnalogButton2::loop()` reads the analog pin once per call, updates `states[]` by matching the reading to each button's registered value with `abs(reading - value) <= tolerance` (inclusive boundary; default tolerance is `ABS_VALUE_RANGE` = 10). First registered match wins — if two buttons overlap, the lower-index one fires, then calls `button.loop()` on every instance — which drives Button2's full state machine (debounce, click/double-click/long-press detection, callbacks).

`add()` returns a `Button2*` (pointer), giving callers direct access to the full Button2 API per button. Returns `nullptr` when the 10-button limit is exceeded — callers must check before dereferencing. Use `->` for chaining: `btns.add(42, "A")->setClickHandler(f)`. String labels are stored in a parallel `ids[]` array and attached to each Button2 instance via `setContext()`; `getId(Button2&)` retrieves them in callbacks.
